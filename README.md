...
社内の会議を一覧表示・検索・登録するための仕組みです。`@kenkohkai.jp` または `@kunimoto-hp.com` の Google Workspace アカウントを持つメンバーが会議の名称・日時・参加者・目的を登録・更新でき、趣旨が似ている会議があれば自動で提案します。

## 採用構成: Google スプレッドシート + Apps Script

検討の結果、**専用アプリは作らず、Google スプレッドシート + Apps Script で実現** する方針です。理由:

- 月額コスト 0 円(既存の Workspace の中で完結)
- サーバー運用・保守が不要(Google が面倒を見てくれる)
- 認証・編集履歴・モバイル対応が標準で備わっている
- 200 名規模ならシートで十分まかなえる

セットアップ手順は **[apps-script/README.md](./apps-script/README.md)** を参照してください。所要 10 分です。

---

## (参考) Web アプリ版

検討段階で Next.js + SQLite による専用 Web アプリも実装しています(コードは本リポジトリに残してあります)。スプレッドシート版で不便が出たり、より複雑な機能が必要になった場合の代替案としてご利用ください。詳細は[こちら](#参考-web-アプリ版-1)。

---

# Google スプレッドシート版(推奨)

→ **[apps-script/README.md](./apps-script/README.md)** を参照
